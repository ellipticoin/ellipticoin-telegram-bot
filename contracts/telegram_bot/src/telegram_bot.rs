...
mod telegram_bot {
    ...
    pub fn deposit(token_deployer: Vec<u8>, token_name: String, to: String, amount: u64) -> Result<Value, Error> {
    ...
    pub fn withdraw(token_deployer: Vec<u8>, token_name: String, to: String, amount: u64) -> Result<Value, Error> {
        let token_address = [&token_deployer, token_name.as_bytes()].concat();
        let (result_code, _result_value) = call(
            token_address.clone(),
            "transfer",
...
mod tests {
    use super::*;
    use ellipticoin::{set_contract_deployer, set_mock_call, set_sender, SYSTEM_ADDRESS};
    ...
    fn test_transfer() {
        set_sender(ALICE.to_vec());
        set_contract_deployer(ALICE.to_vec());
        set_mock_call(
            SYSTEM_ADDRESS.to_vec(),
            "BaseToken",
            "transfer_from",
    ...
    fn test_transfer_is_deployer_only() {
        set_sender(ALICE.to_vec());
        set_contract_deployer(BOB.to_vec());
        set_mock_call(
            SYSTEM_ADDRESS.to_vec(),
            "BaseToken",
            "transfer_from",
